...
function JSXReadError(message, parser$2) {
    var sx = syn.syntaxFromToken({
            type: null,
            value: 'nothing matters',
            lineStart: parser$2.lineStart(),
            lineNumber: parser$2.lineNumber(),
            range: [
                parser$2.index(),
                parser$2.index() + 1
            ]
    ...
    return new SyntaxError(syn.printSyntaxError(parser$2.source(), err));
}
XHTMLEntities = {
    quot: '"',
    amp: '&',
...
var JSXReader = CustomReader.create({
        read: function () {
            ...
            this.add(parser$2.withLoc(parser$2.index(), {
        ...
        readElement: function () {
            ...
            var parser$2 = this.parser;
            var Token = parser$2.Token, selfClosing;
            ...
            var openingName = openingNameToks.slice(0, -1).reduce(tokReduce, '');
            if (!selfClosing) {
                ...
                var closingName = this.getTokens(this.readClosingElement.bind(this)).reduce(tokReduce, '');
                if (openingName !== closingName) {
                    throw new JSXReadError('Expected correspoding closing tag for ' + openingName, this.parser);
                }
            }
        ...
        readOpeningElement: function () {
            ...
            parser$2.scanComment();
            var tokens = this.getTokens(function () {
                    var end = false;
                    while (parser$2.index() < parser$2.length() && !this.match('/') && !this.match('>')) {
                        this.readAttribute();
                        end = this.match('/') || this.match('>');
                        if (!end) {
                            this.add({
                                type: parser$2.Token.Punctuator,
                                value: ','
                            });
                        }
                    }
            ...
            if (tokens.length) {
                this.add(parser$2.withLoc(parser$2.index(), {
                    type: parser$2.Token.Delimiter,
                    value: '{}',
                    inner: tokens
                }));
            } else {
                this.add({
                    type: parser$2.Token.NullLiteral,
                    value: 'null'
                });
            }
            parser$2.scanComment();
        ...
        renderLiteral: function (str) {
            ...
            var parser$2 = this.parser;
            output.forEach(function (str$2, i) {
                if (i !== 0) {
                    this.add({
                        type: parser$2.Token.Punctuator,
                        value: '+'
                    ...
                    this.add({
                        type: parser$2.Token.StringLiteral,
                        value: ' '
                    });
                    this.add({
                        type: parser$2.Token.Punctuator,
        ...
        readText: function (stopChars) {
            var parser$2 = this.parser;
            var ch, str = '';
            var index = parser$2.index();
            var source = parser$2.source();
            var start = index;
            while (index < parser$2.length()) {
                ch = source[index];
                if (stopChars.indexOf(ch) !== -1) {
                    break;
                }
                if (ch === '&') {
                    parser$2.index(index);
                    str += this.readEntity(index);
                    index = parser$2.index();
                } else {
                    index++;
                    if (parser$2.isLineTerminator(ch.charCodeAt(0))) {
                        parser$2.lineNumber(parser$2.lineNumber() + 1);
                        parser$2.lineStart(index);
                    }
                    str += ch;
                }
            }
            parser$2.index(index);
            this.add(parser$2.withLoc(start, {
                type: parser$2.Token.StringLiteral,
                value: str
            }));
        },
        readEntity: function () {
            ...
            index++;
            while (index < parser$2.length() && count < 10) {
                ch = source[index++];
                if (ch === ';') {
                    break;
                }
        ...
        readElementName: function () {
            var parser$2 = this.parser;
            if (!parser$2.isIdentifierStart(parser$2.ch().charCodeAt(0))) {
                throw new JSXBailError('bailed while reading element ' + 'name: ' + parser$2.ch());
            }
            this.readIdentifier();
            if (this.match(':')) {
                this.readPunc();
        ...
        readAttributeValue: function () {
            if (this.match('{')) {
                ...
                if (quote !== '"' && quote !== '\'') {
                    throw new JSXReadError('attributes should be an expression or quoted text', this.parser);
                }
                this.parser.index(start + 1);
                this.readText([quote]);
                if (quote !== this.parser.source()[this.parser.index()]) {
                    throw new JSXReadError('badly quoted string', this.parser);
                }
        ...
        readIdentifier: function () {
            ...
            var source = parser$2.source();
            var ch, start, value = '';
            var chCode = parser$2.ch().charCodeAt(0);
            if (chCode === 92 || !parser$2.isIdentifierStart(chCode)) {
                throw new JSXBailError('bailed while reading identifier: ' + parser$2.ch());
            }
            start = index;
            while (index < parser$2.length()) {
                ch = source.charCodeAt(index);
                // exclude backslash (\) and add hyphen (-)
                if (ch === 92 || !(ch === 45 || parser$2.isIdentifierPart(ch))) {
                    break;
                }
                value += source[index++];
            }
            ...
            this.add(parser$2.withLoc(start, {
                type: parser$2.Token.Identifier,
                value: value
            }));
        }
    });
module.exports = {
    '<': function (ch, parser$2) {
        var reader = new JSXReader(parser$2);
        reader.read();
        var x = reader.finish();
        if (!x.length) {
            return parser$2.advance();
        }
        return x;
    }
};
//# sourceMappingURL=example-readtable.js.map